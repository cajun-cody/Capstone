...
const NewRecipeForm = (props) => {
    ...
    const getCategories = async () => {
      try {
        const response = await axios.get(
          "http://127.0.0.1:8000/api/category/all/",
          {
  ...
  async function addNewRecipe(newRecipe) {
    try {
      let response = await axios.post(
        "http://127.0.0.1:8000/api/recipes/new_recipe/",
        newRecipe,
        {
          headers: {
  ...
  return (
    <section className="new-recipe-section">
      <div className="recipe-header">What Y'all Cookin??</div>
      <form className="recipe-form-container" onSubmit={(e) => handleSubmit(e)}>
        ...
        <div className="user-inputs">
          ...
          <input
            className="recipe-input"
            type="text"
            value={recipeHome_chef}
            onChange={(event) => setRecipeHome_chef(event.target.value)}
          />
        </div>
        <div className="user-inputs">
        ...
        <div className="user-inputs">
          ...
          <input
            className="recipe-input"
            type="number"
            value={recipeServing_size}
            onChange={(event) => setRecipeServing_size(event.target.value)}
          />
        </div>
        <div className="user-inputs">
          <label>Image</label>
