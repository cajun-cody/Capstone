...
const NewIngredientsForm = ({ recipeId }) => {
  ...
  async function addNewRecipeIngredient() {
    try {
      let recipeIngredientResponse = await axios.post(
        `http://127.0.0.1:8000/api/recipe_ingredient/recipes/${recipeId}/ingredients/${ingredient}/add/`,
        {
  ...
  async function listIngredients() {
    try {
      let ingredientsResponse = await axios.get(
        `http://127.0.0.1:8000/api/recipes/all_ingredients/${recipeId}/`,
        {
          headers: {
            Authorization: "Bearer " + token,
          },
  ...
  return (
    <section className="add-ingredient-section" onSubmit={handleSubmit}>
      <h1 className="recipe-header">Add your ingredients here, sha!</h1>
      <form className="add-ingredient-container">
        ...
        <div className="new-ingredient-btn">
          <button className="add-recipe-button" type="submit" onClick={refresh}>
            Add Ingredient
          </button>
        </div>
      </form>
      ...
      <button className="add-recipe-button" onClick={handleViewNewRecipe}>
        View New Recipe
      </button>
    </section>
  );
